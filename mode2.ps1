...
$throttle_interval = 0.1     # минимальный интервал между изменениями тяги (сек)
...
Clear-Host
Write-Host "Посадка на Луну: Режим 2: Suicide Burn" -ForegroundColor Cyan
...
# Функция расчета дистанции торможения (Математический расчет)
# Возвращает необходимую дистанцию для полной остановки с учетом лага и рампы двигателя.

function Get-BrakingDistance($v, $m_fuel) {
    # Глобальные константы используются напрямую
    $m_total = $m_dry + $m_fuel
    
    # 1. Задержка (Lag) - 2.0 сек
    $t1 = 2.0
    $v0 = [Math]::Abs($v)
    
    $d1 = $v0 * $t1 + 0.5 * $g_moon * $t1 * $t1
    $v1 = $v0 + $g_moon * $t1
    
    # 2. Раскрутка (Ramp) - время рассчитывается из throttle_interval и шага тяги
    # От t_min_pct (10%) до t_max_pct (60%) за (60-10)/5 = 10 шагов по 0.1 сек = 1.0 сек
    $t2 = 1.0
    $f_ramp = 0.35 * $t_max  # средняя тяга между 10% и 60%
    $a_ramp = ($f_ramp / $m_total) - $g_moon
    
    $d2 = $v1 * $t2 - 0.5 * $a_ramp * $t2 * $t2
    $v2 = $v1 - $a_ramp * $t2
    
    # 3. Полная тяга (Full Burn)
    if ($v2 -lt 0) { $v2 = 0 } 
    
    $m_final_est = $m_total * [Math]::Exp(-$v2 / $v_e)
    $m_avg = ($m_total + $m_final_est) / 2
    
    $a_burn = ($t_max / $m_avg) - $g_moon
    
    $d3 = ($v2 * $v2) / (2 * $a_burn)
    
    $total_dist = $d1 + $d2 + $d3
    
    # Запас на дискретность симуляции и переход в терминальную фазу
    return $total_dist * 1.05
}
...
while ($h -gt 0) {
    ...
    # --- ЛОГИКА SUICIDE BURN (АВТОМАТИКА) ---

    $t_pct_target = 0

    if ($burn_started) {
        if ($terminal_tracking -or ($v -gt -2.0)) {
            # ФАЗА 2: ТЕРМИНАЛЬНАЯ ПОСАДКА
            # Если мы погасили скорость (V > -2.0), переходим на точное удержание -0.5 м/с
            $terminal_tracking = $true
            
            # Адаптивная скорость снижения:
            # На 400м -> -20.5 м/с
            # На 20м  -> -1.5 м/с
            # На 0м   -> -0.5 м/с
            $v_desired = -0.5 - ($h / 20.0)
            if ($v_desired -lt -40) { $v_desired = -40 }
            ...
            $req_thrust = $hover_thrust + ($v_err * $kp)
            $t_pct_target = [Math]::Round($req_thrust * 100 / $t_max)
            
            # Ограничиваем
            if ($t_pct_target -gt 100) { $t_pct_target = 100 }
            if ($t_pct_target -lt 0) { $t_pct_target = 0 }

            # Не даем движку заглохнуть (idle) если мы в полете
            if ($t_pct_target -lt 10) { $t_pct_target = 10 }
        }
        else {
            # ФАЗА 1: ПОЛНОЕ ТОРМОЖЕНИЕ
            $t_pct_target = 100
        }
    }
    else {
        ...
        if ($v -lt -2.0) {
            $required_dist = Get-BrakingDistance $v $m_fuel
            
            # Если текущая высота подошла к расчетной с запасом 5м
            if ($h -le ($required_dist + 5.0)) {
                $burn_started = $true
                $t_pct_target = 100
            }
        }
    ...
    if ($t_pct_target -eq 0) {
        if (-not $should_shutdown) {
            # просто летим с выключенным двигателем
            # (логика ниже сама переведет это в команду 0 или выключение)
        }
    }
    else {
        if ($t_pct -eq 0) {
...
$outputPath = Join-Path $PSScriptRoot "mode2.png"
